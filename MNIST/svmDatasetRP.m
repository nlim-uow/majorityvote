...
nTrain=size(label0Idx,2)+size(label1Idx,2);
...
y=[ -ones(size(label0Idx,2),1); ones(size(label1Idx,2),1)];
...
label0Idx=indices(labels==label0);
label1Idx=indices(labels==label1);
nTest=size(label0Idx,2)+size(label1Idx,2);
...
yTest=[ -ones(size(label0Idx,2),1); ones(size(label1Idx,2),1)];
vData=tData;
yVal=yTest;
nVal=nTest;
...
i=0;
    ...
    drawnow;
    fileName=sprintf('weightedVote_MNIST_svmRP.fig');
    savefig(fileName);
    fileName=sprintf('weightedVote_MNIST_svmRP.eps');
...
%    psi2=sqrt(r2(sNo,thetaNo,kNo))/(1-sqrt(r2(sNo,thetaNo,kNo)));   
 ...
 %       myPlots(6)=plot(rMLE,'-.');
    ...
    legend(myPlots,legendCell,'Location',[0.4 0.045 0.3 0.04]);
    drawnow;
    fileName=sprintf('polyaModel_MNIST_svmRP.fig');
    savefig(fileName);
    fileName=sprintf('polyaModel_MNIST_svmRP.eps');
    saveas(gcf,fileName,'epsc');

    
  save MNIST_svmRP.mat  
