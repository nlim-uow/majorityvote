...
dataStore=zeros(nTest,dim);
j=0;
for (i=label0Idx)
   j=j+1;
   dataStore(j,:)=reshape(imgs(:,:,i),1,dim);  
end
for (i=label1Idx)
   j=j+1;
   dataStore(j,:)=reshape(imgs(:,:,i),1,dim);  
end
yStore=[ -ones(size(label0Idx,2),1); ones(size(label1Idx,2),1)];
