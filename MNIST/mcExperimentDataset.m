...
valSize=10000;
dim=20*20;
[imgs labels]=readMNIST('train-images.idx3-ubyte','train-labels.idx1-ubyte',trainSize,0);
indices=1:trainSize;

label0Idx=indices(labels==label0);
label1Idx=indices(labels==label1);
label2Idx=indices(labels==label2);
label3Idx=indices(labels==label3);
label4Idx=indices(labels==label4);
nTrain=size(label0Idx,2)+size(label1Idx,2)+size(label2Idx,2);%+size(label3Idx,2);%+size(label4Idx,2);
data=zeros(nTrain,dim);
j=0;
for (i=label0Idx)
   j=j+1;
   data(j,:)=reshape(imgs(:,:,i),1,dim);  
end
%for (i=label3Idx)
%   j=j+1;
%   tData(j,:)=reshape(imgs(:,:,i),1,dim);  
%end
for (i=label1Idx)
   j=j+1;
   data(j,:)=reshape(imgs(:,:,i),1,dim);  
...
classes=[-1 0 1 ];
%y=[ -2*ones(size(label0Idx,2),1); -ones(size(label3Idx,2),1); zeros(size(label1Idx,2),1); ones(size(label2Idx,2),1); 2*ones(size(label4Idx,2),1)];
y=[ -ones(size(label0Idx,2),1); zeros(size(label1Idx,2),1); ones(size(label2Idx,2),1)];


[imgs labels]=readMNIST('t10k-images.idx3-ubyte','t10k-labels.idx1-ubyte',testSize,0);
indices=1:testSize;
...
label3Idx=indices(labels==label3);
label4Idx=indices(labels==label4);
nTest=size(label0Idx,2)+size(label1Idx,2)+size(label2Idx,2);%+size(label3Idx,2);%+size(label4Idx,2);
tData=zeros(nTest,dim);
j=0;
for (i=label0Idx)
   j=j+1;
   tData(j,:)=reshape(imgs(:,:,i),1,dim);  
...
%yTest=[ -2*ones(size(label0Idx,2),1); -ones(size(label3Idx,2),1); zeros(size(label1Idx,2),1); ones(size(label2Idx,2),1); 2*ones(size(label4Idx,2),1)];
yTest=[ -ones(size(label0Idx,2),1); zeros(size(label1Idx,2),1); ones(size(label2Idx,2),1)];


%yTest=[ -ones(size(label0Idx,2),1); zeros(size(label1Idx,2),1); ones(size(label2Idx,2),1); 2*ones(size(label3Idx,2),1)];
vData=tData;
yVal=yTest;
...
%    end
    ...
    yEnsRec(:,i)=ySub;  
    yEns=multiClassMajVote(yEnsRec(:,1:i),classes);
%   
...
close gcf;
sTitle=sprintf('Ensemble Accuracy vs Ensemble Size for Dataset: MNIST, Training size=%d, Test Size=%d, d=%d',nTrain, nTest,d);
[fig myAxes]=createAxes([2 2],'title',sTitle,'hSize',7.5,'vSize',8.5,'leftOffset',0,'legendHeight',0.75,'vMargin',1.25);
...
i=0;
    ...
    drawnow;
    fileName=sprintf('weightedVote_MNIST_svm_class3.fig');
    savefig(fileName);
    fileName=sprintf('weightedVote_MNIST_svm_class3.eps');
...
%    psi2=sqrt(r2(sNo,thetaNo,kNo))/(1-sqrt(r2(sNo,thetaNo,kNo)));   
 ...
 %       myPlots(6)=plot(rMLE,'-.');
    ...
    legend(myPlots,legendCell,'Location',[0.4 0.045 0.3 0.04]);
    drawnow;
    fileName=sprintf('polyaModel_MNIST_svm_class3.fig');
    savefig(fileName);
    fileName=sprintf('polyaModel_MNIST_svm_class3.eps');
    saveas(gcf,fileName,'epsc');
