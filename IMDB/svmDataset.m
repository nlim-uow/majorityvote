...
close gcf;
sTitle=sprintf('Ensemble Accuracy vs Ensemble Size for Dataset: IMDB, Training size=%d, Test Size=%d, d=%d',nTrain, nTest,d);
...
i=0;
    ...
    drawnow;
    fileName=sprintf('weightedVote_IMDB_svm.fig');
    savefig(fileName);
    fileName=sprintf('weightedVote_IMDB_svm.eps');
...
%    psi2=sqrt(r2(sNo,thetaNo,kNo))/(1-sqrt(r2(sNo,thetaNo,kNo)));   
 ...
 %       myPlots(6)=plot(rMLE,'-.');
    ...
    legend(myPlots,legendCell,'Location',[0.4 0.045 0.3 0.04]);
    drawnow;
    fileName=sprintf('polyaModel_IMDB_svm.fig');
    savefig(fileName);
    fileName=sprintf('polyaModel_IMDB_svm.eps');
    saveas(gcf,fileName,'epsc');
