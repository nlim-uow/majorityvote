...
close gcf;
sTitle=sprintf('Ensemble Accuracy vs Ensemble Size for Dataset: Gisette, Training size=%d, Test Size=%d, d=%d',nTrain, nTest,d);
[fig myAxes]=createAxes([2 2],'title',sTitle,'hSize',8.5,'vSize',11.5,'leftOffset',0,'legendHeight',0.75,'vMargin',1.25);
...
i=0;
    ...
    drawnow;
    fileName=sprintf('weightedVote_gisette.fig');
    savefig(fileName);
    fileName=sprintf('weightedVote_gisette.eps');
...
sTitle=sprintf('Ensemble Accuracy vs Ensemble Size for Gisette');
...
%    psi2=sqrt(r2(sNo,thetaNo,kNo))/(1-sqrt(r2(sNo,thetaNo,kNo)));   
 ...
 %       myPlots(6)=plot(rMLE,'-.');
    ...
    legend(myPlots,legendCell,'Location',[0.4 0.045 0.3 0.04]);
    drawnow;
    fileName=sprintf('polyaModel_gisette.fig');
    savefig(fileName);
    fileName=sprintf('polyaModel_gisette.eps');
    saveas(gcf,fileName,'epsc');
