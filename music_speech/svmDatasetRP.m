...
musicLst=dir('music/*.wav');
for (i=1:size(musicLst,1))
    fileName=musicLst(i).name;
    tempArray=reshape(audioread(strcat('music/',fileName))',22050,30)';
    if(i==1)
        data=tempArray(1:2:30,:);
        tData=tempArray(2:2:30,:);
    else
        data=[data;tempArray(1:2:30,:)];
        tData=[tData;tempArray(2:2:30,:)];   
    end
end
speechLst=dir('speech/*.wav');
for (i=1:size(speechLst,1))
    fileName=speechLst(i).name;
    tempArray=reshape(audioread(strcat('speech/',fileName))',22050,30)';
    data=[data;tempArray(1:2:30,:)];
    tData=[tData;tempArray(2:2:30,:)];   
end
y=[ones(15*size(musicLst,1),1); -ones(15*size(speechLst,1),1)];
yTest=[ones(15*size(musicLst,1),1); -ones(15*size(speechLst,1),1)];
...
i=0;
    ...
    drawnow;
    fileName=sprintf('weightedVote_music_svmRP.fig');
    savefig(fileName);
    fileName=sprintf('weightedVote_music_svmRP.eps');
...
%    psi2=sqrt(r2(sNo,thetaNo,kNo))/(1-sqrt(r2(sNo,thetaNo,kNo)));   
 ...
 %       myPlots(6)=plot(rMLE,'-.');
    ...
    legend(myPlots,legendCell,'Location',[0.4 0.045 0.3 0.04]);
    drawnow;
    fileName=sprintf('polyaModel_music_svmRP.fig');
    savefig(fileName);
    fileName=sprintf('polyaModel_music_svmRP.eps');
    saveas(gcf,fileName,'epsc');

    
  save music_svmRP.mat  
