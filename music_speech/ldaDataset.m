...
i=0;
    ...
    drawnow;
    fileName=sprintf('weightedVote_music.fig');
    savefig(fileName);
    fileName=sprintf('weightedVote_music.eps');
...
%    psi2=sqrt(r2(sNo,thetaNo,kNo))/(1-sqrt(r2(sNo,thetaNo,kNo)));   
 ...
 %       myPlots(6)=plot(rMLE,'-.');
    ...
    legend(myPlots,legendCell,'Location',[0.4 0.045 0.3 0.04]);
    drawnow;
    fileName=sprintf('polyaModel_music.fig');
    savefig(fileName);
    fileName=sprintf('polyaModel_music.eps');
    saveas(gcf,fileName,'epsc');

    
  save musicSpeech.mat  
